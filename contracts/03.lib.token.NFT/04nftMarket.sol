...
contract NFTMarket is ITokenReceiver {
    ...
    function list(
        ...
        IERC721 nftContract = IERC721(_nftContract); // todo 为啥把地址放进接口中
    ...
    function buyNFT(uint256 _listingId) external {
        // 检查上架信息是否存在且处于活跃状态
        Listing storage listing = listings[_listingId]; //todo 为啥用storage
        require(listing.isActive, "NFTMarket: listing is not active");
    ...
    // 实现tokensReceived接口，处理通过transferWithCallback接收到的代币 回调，被动触发当NFTMarket收到代币，就执行代币转账和NFT转让任务 from是买家
    function tokensReceived(
        address from,
        uint256 amount,
        bytes calldata data
        ...
        // 检查调用者是否为支付代币合约
        require(
            msg.sender == address(paymentToken), //msg.sender是调用paymentToken.transferWithCallbackAndData函数的发起者即paymentToken
    ...
    function buyNFTWithCallback(uint256 _listingId) external {
        ...
        // 调用transferWithCallbackAndData函数，将代币转给市场合约并附带listingId数据  第一个参数指的是把钱转到NFTMarket合约中，用于回调this地址中的tokenReceived方法
